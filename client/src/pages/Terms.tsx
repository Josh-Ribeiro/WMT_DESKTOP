...
export default function Terms() {
  ...
  const [termType, setTermType] = useState<"responsibility" | "return">(
    "responsibility"
  );
  ...
  const [printPreview, setPrintPreview] = useState<GeneratedDocument | null>(
    null
  );
  ...
  const handleOpenForPrint = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to prepare print preview.";
      setStatus(message);
      toast.error(message);
    } finally {
      setPrinting(false);
...
<head>
  ...
  <style>
    ...
    .wmt-machine-term-preview-wrapper { padding: 0 !important; background: #fff !important; }
    @page { margin: 12mm; }
    @media print {
      html, body { margin: 0 !important; padding: 0 !important; }
      .wmt-machine-term-preview-wrapper { margin: 0 !important; padding: 0 !important; }
    }
  </style>
</head>
<body>${container.innerHTML}</body>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setStatus(
        missing
          ? `DOCX ready. Unused placeholders: ${missing}`
    ...
    <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      <main className="h-full min-w-0 flex-1 overflow-auto">
        <div className="mx-auto flex max-w-6xl flex-col gap-8 p-6 lg:p-8">
          <header className="wmt-header rounded-xl border p-5 text-slate-100 shadow-lg">
            <h1 className="text-3xl font-bold text-white">Terms</h1>
            <p className="mt-1 text-sm text-slate-400">
              Generate responsibility and return documents from workstation
              inventory data.
            </p>
          </header>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_180px]">
                <label className="space-y-2">
                  ...
                  <Select
                    value={termType}
                    onValueChange={value =>
                      setTermType(value as "responsibility" | "return")
                    }
                  >
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    WMT reads the WK data and prepares the DOCX. Use Download
      ...
      {printPreview && (
        <div className="fixed inset-0 z-50 flex flex-col bg-background/95 backdrop-blur-sm">
          <div className="flex items-center justify-between gap-3 border-b border-border bg-card px-5 py-3">
            ...
            <div className="flex shrink-0 gap-2">
              <Button
                variant="outline"
                className="gap-2"
                onClick={() => downloadDocument(printPreview)}
              >
                <Download size={16} />
          ...
          <div className="min-h-0 flex-1 overflow-auto bg-zinc-200 p-4 dark:bg-zinc-900">
            ...
            <div className="mx-auto min-h-full max-w-[960px] rounded bg-white p-3 text-black shadow-sm">
              {previewRendering && (
              ...
              <div
                ...
                className={
                  previewRendering
                    ? "hidden"
                    : "cursor-text rounded outline-none focus-within:ring-2 focus-within:ring-primary/40"
                }
              />
            </div>
          </div>
