...
export default function Account() {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      error(
        err instanceof Error ? err.message : t("Failed to change password")
      );
    }
  };
    ...
    <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      <main className="h-full min-w-0 flex-1 overflow-auto">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-6 lg:p-8">
          <section className="wmt-header flex flex-col gap-4 rounded-xl border p-6 text-slate-100 shadow-lg md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col gap-6">
              <Card className="rounded-lg border-border/70 shadow-none">
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("Current identity used by WMT.")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {user.permissions?.length ? (
                        ...
                        <span className="text-sm text-muted-foreground">
                          {t("No explicit permissions loaded.")}
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <Card className="rounded-lg border-border/70 shadow-none">
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("Current authentication state.")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="rounded-lg border border-border/70 bg-muted/40 p-4">
                    <p className="text-sm font-medium text-foreground">
                      {isWindowsSession
                        ? t("Authenticated with your Windows account.")
                        : t("Authenticated with a local WMT account.")}
                    </p>
            ...
            <div className="flex flex-col gap-6">
              <Card className="rounded-lg border-border/70 shadow-none">
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("Choose how WMT should look on this workstation.")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="flex flex-col gap-4 rounded-lg border border-border/70 p-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <select
                      value={language}
                      onChange={event =>
                        setLanguage(event.target.value as "en-US" | "pt-BR")
                      }
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                      {accentOptions.map(option => {
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`flex size-8 shrink-0 items-center justify-center rounded-md ${option.className} text-white`}
                            >
            ...
            <Card className="rounded-lg border-border/70 shadow-none">
              <CardHeader>
                <button
                  ...
                  onClick={() => setAdminSettingsOpen(open => !open)}
