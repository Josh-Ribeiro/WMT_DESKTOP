...
const hiddenRemoteActions = new Set([
  "create-temp-c-share",
  "remove-temp-c-share",
]);
      ...
      {items.slice(0, 6).map(item => (
        ...
        >
          ...
          <p className="mt-1 truncate text-xs text-muted-foreground">
            {item.username || "-"}{" "}
            {item.details?.filename ? `- ${String(item.details.filename)}` : ""}
          </p>
        </div>
      ))}
    </div>
...
export default function WorkstationHistory() {
  ...
  const visibleEvents = (data?.events || []).filter(
    event => !hiddenRemoteActions.has(event.title)
  );
  const visibleRemoteActions = (data?.remote_actions || []).filter(
    ...
    <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      <main className="h-full min-w-0 flex-1 overflow-auto">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 p-6 lg:p-8">
          <header className="wmt-header flex flex-col gap-4 rounded-xl border p-5 text-slate-100 shadow-lg lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white">
                Workstation History
              </h1>
              <p className="mt-1 text-sm text-slate-400">
                Histórico operacional por WK: backups, ações remotas,
                diagnósticos e termos.
              </p>
            </div>
            ...
            <>
              <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <StatTile
                  ...
                  value={visibleRemoteActions.length}
                />
                <StatTile
                  label="Diagnostics"
                  value={summary?.diagnostics || 0}
                />
              ...
              <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
                <div className="space-y-5">
                        ...
                        <div className="divide-y divide-border rounded-md border border-border">
                          {visibleEvents.map(event => (
                            <div
                              ...
                              className="grid gap-3 p-3 md:grid-cols-[150px_minmax(0,1fr)_auto] md:items-center"
                            >
                  ...
                  <div className="grid gap-5 lg:grid-cols-2">
                    <Card>
                      <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                          <HardDrive size={18} /> Backups
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-2">
                        {data.backups.length ? (
                          data.backups.slice(0, 6).map(job => (
                            <div
                              key={job.id}
                              className="rounded-md border border-border/70 bg-background px-3 py-2 text-sm"
                            >
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                          <TerminalSquare size={18} /> Remote actions
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-2">
                        {visibleRemoteActions.length ? (
                          visibleRemoteActions.slice(0, 6).map(job => (
                            <div
                              key={job.id}
                              className="rounded-md border border-border/70 bg-background px-3 py-2 text-sm"
                            >
                              ...
                              <p className="mt-1 truncate text-xs text-muted-foreground">
                                ...
                                {job.message || job.created_by || "-"}
                              </p>
                            </div>
                          ))
                        ) : (
                          <p className="text-sm text-muted-foreground">
                            Nenhuma ação remota relacionada.
                          </p>
                        )}
                      </CardContent>
                    </Card>
                ...
                <aside className="space-y-5">
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                        <AlertTriangle size={18} /> Erros recentes
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
