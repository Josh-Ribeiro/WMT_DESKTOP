...
export default function Login() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const text = error instanceof Error ? error.message : 'Falha ao autenticar';
  ...
  const handleSsoRetry = async () => {
    ...
    try {
      ...
      const text = error instanceof Error ? error.message : 'Windows SSO indisponível';
      setErrorMessage(text);
      toast.error(text);
    } finally {
      setRetryingSso(false);
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-3 text-center">
          ...
          <CardDescription>Conectando com sua sessão do Active Directory</CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          <Button
            ...
            disabled={busy}
          >
            {retryingSso ? <Loader2 className="animate-spin" size={16} /> : <RefreshCw size={16} />}
