...
import { Plus, Edit2, Trash2, Lock, Unlock, Search, Loader2 } from 'lucide-react';
...
export default function AdminUsers() {
  ...
  const handleSubmitUser = (payload: Partial<User> & { password?: string }) => {
    ...
    toast.promise(
      request.then(() => refetch()),
      {
        ...
        error: (err) => (err instanceof Error ? err.message : 'Failed to save user'),
      }
    );
  };
  ...
  const handleDeleteUser = (userId: string) => {
    toast.promise(
      apiRequest(`/api/users/${userId}`, { method: 'DELETE' }).then(() => refetch()),
      {
                        ...
                        >
                          <td className="py-3 px-4 text-foreground font-medium">
                            <div className="min-w-0">
                              ...
                              <p className="text-xs font-normal text-muted-foreground">{u.domain ? `${u.domain}\\${u.username}` : u.username}</p>
                            </div>
                          </td>
                          ...
                          <td className={`py-3 px-4 font-medium ${getStatusColor(u.status)}`}>
