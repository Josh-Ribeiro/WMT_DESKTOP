...
function isInstallableSoftware(app: Record<string, unknown>) {
  ...
  if (
    ...
    (name.startsWith("windows ") || name.startsWith("microsoft windows"))
  )
    return false;
  return true;
}
  ...
  return new Map(
    apps
      ...
      .map(app => {
        ...
        return [
          name.toLowerCase(),
          { name, version: String(app.DisplayVersion || "") },
        ] as const;
      })
...
async function waitForDiagnostic(job: DiagnosticJob): Promise<DiagnosticJob> {
  ...
  while (
    ...
    current = await apiRequest<DiagnosticJob>(
...
export default function MachineReplacement() {
  ...
  const loadProfiles = async () => {
    ...
    try {
      const result = await apiRequest<{ users: string[]; count: number }>(
        "/api/backup/users",
        {
          method: "POST",
      ...
      toast.error("Falha ao identificar perfis", {
        description: err instanceof Error ? err.message : normalizeHost(source),
      });
    } finally {
  ...
  const compareSoftware = async () => {
    ...
    try {
      ...
      const [sourceJob, destinationJob] = await Promise.all([
        create(source),
        create(destination),
      ]);
      const [sourceResult, destinationResult] = await Promise.all([
        waitForDiagnostic(sourceJob),
        waitForDiagnostic(destinationJob),
      ]);
      if (
        sourceResult.status !== "completed" ||
        destinationResult.status !== "completed"
      ) {
        throw new Error(
          sourceResult.error ||
            destinationResult.error ||
            "Não foi possível coletar os dois inventários."
        );
      }
      ...
      const names = Array.from(
        new Set([
          ...Array.from(sourceApps.keys()),
          ...Array.from(destinationApps.keys()),
        ])
      ).sort();
      setSoftwareRows(
        names.map(name => ({
          name:
            (sourceApps.get(name) || destinationApps.get(name))?.name || name,
          sourceVersion: sourceApps.get(name)?.version || "",
          destinationVersion: destinationApps.get(name)?.version || "",
          presence:
            sourceApps.has(name) && destinationApps.has(name)
              ? "both"
              : sourceApps.has(name)
                ? "source"
                : "destination",
      ...
      toast.error("Falha ao comparar softwares", {
        description: err instanceof Error ? err.message : "Erro desconhecido",
      });
    } finally {
    ...
    const poll = async () => {
      try {
        const job = await apiRequest<BackupJob>(
          `/api/backup/jobs/${encodeURIComponent(jobId)}`
        );
        if (cancelled) return;
        setBackupJob(job);
        if (["queued", "running"].includes(job.status)) {
          ...
          if (job.status === "completed") {
            toast.success("Migração concluída", {
              description: job.summary || job.id,
            });
            setStep(5);
          } else {
            const message =
              job.message || job.summary || "O job terminou com falha.";
            setCopyError(message);
        ...
        if (cancelled) return;
        setCopyError(
          err instanceof Error ? err.message : "Falha ao atualizar o job."
        );
  ...
  const generateTerm = async () => {
    setTermLoading(true);
    try {
      const response = await apiFetch("/api/terms/generate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const filename =
        ...
        `${normalizeHost(destination)}-responsibility.docx`;
      ...
      toast.error("Falha ao gerar termo", {
        description: err instanceof Error ? err.message : "Erro desconhecido",
      });
    } finally {
  ...
  useEffect(() => {
    ...
    import("docx-preview")
      ...
      .then(() => {
        ...
        container
          .querySelectorAll<HTMLElement>(
            ".wmt-machine-term-preview-wrapper, .wmt-machine-term-preview, section"
          )
          .forEach(element => {
      ...
      .catch(err => {
        if (cancelled) return;
        setTermPreviewRendering(false);
        toast.error("Falha ao renderizar o termo", {
          description: err instanceof Error ? err.message : "DOCX inválido",
        });
      });
    return () => {
      cancelled = true;
  ...
  const printEditedTerm = () => {
    const container = termPreviewRef.current;
    const styleContainer = termStyleRef.current;
    if (!container?.innerHTML.trim() || !styleContainer || termPreviewRendering)
      return;
    ...
    () =>
      softwareRows.filter(
        row =>
          row.presence === "source" ||
          (row.presence === "both" &&
            row.sourceVersion !== row.destinationVersion)
      ),
    [softwareRows]
  );
      ...
      const url = await requestPdf("/api/machine-replacement/report", {
        ...
        applications: applicationsToInstall.map(row => ({
          ...
          action:
            row.presence === "source"
              ? "Instalar no destino"
              : `Atualizar para ${row.sourceVersion || "versao da origem"}`,
        })),
      });
  ...
  const canAdvance =
    step === 0
      ? Boolean(
          source.trim() &&
            destination.trim() &&
            normalizeHost(source) !== normalizeHost(destination)
        )
      : step === 1
    ...
    <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      <main className="h-full min-w-0 flex-1 overflow-auto">
        <div className="mx-auto w-full max-w-6xl space-y-5 p-6 lg:p-8">
          <section className="wmt-header relative overflow-hidden rounded-xl border p-6 text-slate-100 shadow-lg">
            ...
            <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-slate-400">
                  Uma migração guiada, com validações rápidas e acompanhamento
                  visível do início ao fim.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-xl border bg-background/75 px-4 py-3">
                  <p className="text-muted-foreground">Origem</p>
                  <p className="mt-1 font-semibold">
                    {normalizeHost(source) || "A definir"}
                  </p>
                </div>
                <div className="rounded-xl border bg-background/75 px-4 py-3">
                  <p className="text-muted-foreground">Destino</p>
                  <p className="mt-1 font-semibold">
                    {normalizeHost(destination) || "A definir"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-7">
            {steps.map(([title], index) => (
              <button
                ...
                className={`group rounded-xl border p-3 text-left transition-all ${index === step ? "border-primary bg-primary text-primary-foreground shadow-md shadow-primary/15" : index < step ? "border-emerald-500/40 bg-emerald-500/5 hover:bg-emerald-500/10" : "border-border bg-card opacity-65"}`}
              >
                <span
                  className={`mb-2 flex size-6 items-center justify-center rounded-full text-xs font-bold ${index === step ? "bg-primary-foreground/20" : index < step ? "bg-emerald-500 text-white" : "bg-muted"}`}
                >
                  {index < step ? "✓" : index + 1}
                </span>
            ...
            <CardContent className="space-y-4">
              {step === 0 && (
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <label className="space-y-2 text-sm">
                    <span>Destino da migração</span>
                    <Input
                      value={destinationPath}
                      onChange={event => setDestinationPath(event.target.value)}
                      placeholder="D:\\Backup\\Migration"
                    />
                  </label>
                </div>
              )}
                ...
                <>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground">
                      Sem varredura profunda das pastas; conectividade, acesso,
                      perfis e disco continuam sendo verificados.
                    </span>
                  </div>
                  {precheckLoading && (
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-sm">
                          {precheck.quick
                            ? "Validação rápida"
                            : `Estimativa: ${precheck.estimated_size}`}
                        </span>
                      </div>
              ...
              {step === 3 && (
                <>
                  <Button onClick={compareSoftware} disabled={softwareLoading}>
                    {softwareLoading ? (
                      <Loader2 className="animate-spin" size={16} />
                    ) : (
                      <GitCompare size={16} />
                  ...
                  <div className="max-h-[430px] overflow-auto rounded-lg border">
                    {softwareDifferences.map(row => (
                      <div
                        key={row.name}
                        className="grid grid-cols-[minmax(0,1fr)_130px_130px] gap-3 border-b px-3 py-2 text-sm last:border-0"
                      >
                        <span className="truncate font-medium">{row.name}</span>
                        <span>{row.sourceVersion || "Não instalado"}</span>
                        <span>{row.destinationVersion || "Não instalado"}</span>
                      </div>
                    ))}
                    {softwareCompared && !softwareDifferences.length && (
              ...
              {step === 4 && (
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    ...
                    >
                      <RefreshCw size={16} />
                      Abrir central de backups
                    </Button>
                  </div>
                  {copyError && (
                    <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-4 text-sm text-red-600 dark:text-red-300">
                      <p className="font-semibold">
                        O job encontrou um problema
                      </p>
                      <p className="mt-1">{copyError}</p>
                    </div>
                  )}
                  {backupJob && (
                    <div className="space-y-4 rounded-xl border bg-muted/15 p-5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-2 text-sm font-semibold">
                            {backupJob.message || "Job registrado no backend"}
                          </p>
                        </div>
                      ...
                      <div className="grid gap-3 text-xs sm:grid-cols-3">
                        <div className="rounded-lg bg-background p-3">
                          ...
                          <p className="mt-1 font-semibold">
                            {backupJob.current_step || 0}/
                            {backupJob.total_steps || "—"}
                          </p>
                        </div>
                        <div className="rounded-lg bg-background p-3">
                          ...
                          <p className="mt-1 flex items-center gap-1 font-semibold">
                            <RefreshCw
                              className={
                                ["queued", "running"].includes(backupJob.status)
                                  ? "animate-spin"
                                  : ""
                              }
                              size={13}
                            />
                            Automática a cada 1,5s
                          </p>
                        </div>
                        <div className="rounded-lg bg-background p-3">
                          <p className="text-muted-foreground">
                          ...
                          <p className="mt-1 flex items-center gap-1 font-semibold">
                            <Clock3 size={13} />
                            {backupJob.eta_seconds
                              ? `${Math.ceil(backupJob.eta_seconds / 60)} min`
                              : "Calculando"}
                          </p>
                        </div>
                      </div>
                      {backupJob.log && (
                        <pre className="max-h-36 overflow-auto whitespace-pre-wrap rounded-lg bg-zinc-950 p-3 text-[11px] text-zinc-200">
                          {backupJob.log.split(/\r?\n/).slice(-8).join("\n")}
                        </pre>
                      )}
                      {backupJob.failures?.length ? (
                        <p className="text-sm text-red-500">
                          {backupJob.failures.slice(0, 3).join(" • ")}
                        </p>
                      ) : null}
                    </div>
                  )}
                ...
                <div className="space-y-4">
                  <div className="rounded-lg border border-emerald-500/40 bg-emerald-500/5 p-4">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {backupJob?.validation?.message ||
                        backupJob?.summary ||
                        "A rotina terminou sem bloqueios."}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button onClick={generateTerm} disabled={termLoading}>
                      {termLoading ? (
                        <Loader2 className="animate-spin" size={16} />
                      ) : (
                        <FileText size={16} />
                      )}
                      {termGenerated ? "Gerar novamente" : "Gerar termo"}
                    </Button>
                    <Button
                      variant="outline"
                      disabled={!termDocument}
                      title={
                        !termDocument
                          ? "Gere o termo primeiro"
                          : "Baixar DOCX original"
                      }
                      onClick={() =>
                        termDocument &&
                        downloadPdf(termDocument.url, termDocument.filename)
                      }
                    >
                      <Download size={16} />
                      Baixar DOCX
                    ...
                    <Button
                      variant="outline"
                      disabled={!termDocument || termPreviewRendering}
                      title={
                        !termDocument
                          ? "Gere o termo primeiro"
                          : "Imprimir conteúdo editado"
                      }
                      onClick={printEditedTerm}
                    >
                      <Printer size={16} />
                      Imprimir editado
                    </Button>
                  ...
                  {!termDocument && (
                    <p className="text-sm text-muted-foreground">
                      O WMT carregará o mesmo termo de responsabilidade e
                      aceitação configurado no módulo Terms.
                    </p>
                  )}
                  {termDocument && (
                    <div className="overflow-hidden rounded-xl border bg-muted/20">
                      <div className="flex flex-wrap items-center justify-between gap-2 border-b px-4 py-2 text-xs text-muted-foreground">
                        <span>{termDocument.filename}</span>
                        <div className="flex items-center gap-2">
                          <span>
                            Clique no documento para editar antes de imprimir.
                          </span>
                          <Badge variant="outline">DOCX da rede</Badge>
                        </div>
                      </div>
                      <div ref={termStyleRef} />
                      {termPreviewRendering && (
                        <div className="flex h-40 items-center justify-center gap-2 text-sm text-muted-foreground">
                          <Loader2 className="animate-spin" size={16} />
                          Renderizando documento...
                        </div>
                      )}
                      <div className="max-h-[680px] overflow-auto bg-zinc-200 p-4 dark:bg-zinc-900">
                        <div
                          ref={termPreviewRef}
                          className={
                            termPreviewRendering
                              ? "hidden"
                              : "mx-auto max-w-[960px] cursor-text rounded bg-white p-3 text-black outline-none focus-within:ring-2 focus-within:ring-primary/40"
                          }
                        />
                      </div>
                    </div>
              ...
              {step === 6 && (
                <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          ...
                          {applicationsToInstall.length} aplicativo(s) que
                          exigem instalação ou atualização.
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <Button
                        ...
                        >
                          ...
                          {reportPdfUrl ? "Atualizar PDF" : "Gerar PDF"}
                        </Button>
                        <Button
                          variant="outline"
                          ...
                          onClick={() =>
                            downloadPdf(
                              reportPdfUrl,
                              `troca-${normalizeHost(source)}-${normalizeHost(destination)}.pdf`
                            )
                          }
                        >
                          <Download size={16} />
                  ...
                  {reportLoading && (
                    <div className="flex items-center justify-center gap-3 rounded-xl border py-16 text-sm text-muted-foreground">
                      <Loader2 className="animate-spin" size={20} />
                      Montando relatório e convertendo para PDF...
                    </div>
                  )}
                  {reportPdfUrl && !reportLoading && (
                    <div className="overflow-hidden rounded-xl border bg-muted/20">
                      <div className="flex items-center justify-between border-b px-4 py-2 text-xs text-muted-foreground">
                        <span>Relatório da troca de máquina</span>
                        <Badge variant="outline">Exibido no WMT</Badge>
                      </div>
                      <iframe
                        ref={reportFrameRef}
                        title="Relatório da troca de máquina"
                        src={reportPdfUrl}
